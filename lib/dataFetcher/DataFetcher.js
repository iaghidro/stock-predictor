...
class DataFetcher {
  async fetchMulti(pairs, lookbackWindow = 1, exchange) {
    ...
    pairs.map((pair, index) =>
      queries.push(this.fetch(pair, lookbackWindow, 1000 * index))
    );
  ...
  async fetch(pair, lookbackWindow = 1, delay = 0) {
    await util.delay(delay);
    const historicalRows = await this.query(
      pair,
      this.createConfig(lookbackWindow)
    );
    const processed = await this.processResponse(pair, historicalRows);
    return processed;
  }

  createConfig(lookbackWindow) {
