...
class StockDataProcessor {
    ...
    validateData(jsonData) {
        ...
        _.map(jsonData, (dataItem, index, list) => {
            ...
            const currentDate = moment(dataItem.time_period_start);
    ...
    processFields(jsonData) {
        ...
        const priceHighSeries = {
            start: moment(jsonData[0].time_period_start).format('YYYY-MM-DDTHH:mm:ss'),
            target: _.map(jsonData, (dataItem) => dataItem.price_high)
        };
        const cleanDataItem = [
            priceHighSeries,
        ];
