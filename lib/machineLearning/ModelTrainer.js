...
const bucketName = 'istvan-ml-model';
const trainingName = 'ethUsdCoinbase'; //todo needs to be config

const machineLearningInputFileName = 'historicalData.csv'; //todo: refactor input/output files

const sageMakerInputFileName = 'historicalData.json';
const sageMakerOutputDirectoryName = 'historicalDataOutput';

const coinAPIKeys = [
    "3DA8C66B-8A17-47DB-8C1D-B765C2D1C0D1",
    "8E92EF26-A47F-492E-9C6A-585965D85C53",
    "12B37907-7AA9-4FBD-BC59-EA200011EE08"
];
...
class ModelTrainer {
    ...
    constructor(params = {}) {
        ...
        const machineLearningParams = {
            bucketName,
            inputFileName: machineLearningInputFileName,
            region: 'us-east-1'
        };
...
//                .then((data) => this.saveToFile(data))
                ...
                .then(() => this.machineLearning.train(trainingName, mlConfig.dataSchemas.base))
    }
...
//                .then((data) => this.saveToFile(data))
                ...
                .then(() => this.sageMaker.train(trainingName))
    }

    fetchFakeData(params = {}) {
        console.log(`ModelTrainer::fetchFakeData`);
        const fakeData = require('./stockTrainingData');
...
//            remove first set of bad data
    ...
    saveCsv(data = []) {
        ...
        return this.s3.uploadFile({
            bucketName,
            file: data,
            fileName: machineLearningInputFileName
        });

//        const filePath = "/app/historicalData.csv";
//        return new Promise((resolve, reject) => {
//            createFile(filePath, data, function (err) {
//                if (err) {
//                    return reject(err);
//                }
//                resolve();
