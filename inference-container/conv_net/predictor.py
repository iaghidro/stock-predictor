...
class ClassificationService(object):
    ...
    def get_model(cls):
        ...
        if cls.model == None:
            data = cls.get_data()
            cls.model = ConvLearner.pretrained(
                ARCH, data, precompute=False, models_name='model')
    ...
    def predict(cls, img_path):
        ...
        # Pick the index with highest log probability
        maxP = np.argmax(test_preds, axis=1)
